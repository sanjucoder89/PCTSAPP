...
import 'child_infant_list_details.dart';
import 'model/GetChildDeathListData.dart';

void main(){
    runApp(ShishuDeathList());
}
...
class ShishuDeathList extends StatefulWidget {

  @override
  State<StatefulWidget> createState() => _ShishuDeathList();
...
class _ShishuDeathList extends State<ShishuDeathList> {
  ...
  var _get_ShishuDeath_list = AppConstants.app_base_url+"InfantDeathList";
  ...
  * */
  Future<String> getLisingAPI(String village_id) async {
    ...
    var resBody = json.decode(response.body);
    final apiResponse = GetChildDeathListData.fromJson(resBody);
    setState(() {
      if (apiResponse.status == true) {
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
              builder: (BuildContext context) =>
                  ChildInfantList(pctsID: id), //TabViewScreen ,VideoScreen
            ));
      } else {
        Fluttertoast.showToast(
            msg:apiResponse.message.toString(),
